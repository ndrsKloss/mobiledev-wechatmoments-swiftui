...
| `NFR-LAYOUT-003` | MUST | The image grid's cell size **MUST** derive from available width (`FR-TWEET-010`), preserving square cells and consistent gutters. *`View/ImageGridView.swift` draws every `fn-spec §5` layout inside one three-column `Grid`, so nothing computes a width: a cell is a third of the space offered, at any device size or orientation. Gutters come from `Constants.TWEET_IMAGE_SEPARATOR_SPACE` — a gutter is intrinsic, a width is not. It is built on `Grid`/`GridRow` rather than the `LazyVGrid` §3.5 suggests: `LazyVGrid` has no column span and flows its items continuously, so it can express neither `fn-spec §5`'s single large image nor its 2×2 block. §3.5's claim holds for the three-column rows; it is simply not sufficient for all six.* |
